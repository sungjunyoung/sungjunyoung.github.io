...
if (container && !container.querySelector("script")) {
  ...
  // Fires in other tabs, where theme.ts never touched this document's body
  // class, so this only matters for the theme the toggle stored.
  window.addEventListener("storage", pushTheme);

  window.addEventListener("message", (event) => {
    if (event.origin !== GISCUS_ORIGIN || giscusReady) return;
    giscusReady = true;
    // Flush whatever the page settled on while the frame was still lazy.
    sendTheme();
  });
}
